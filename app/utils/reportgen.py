...
from reportlab.platypus import Frame, PageTemplate, NextPageTemplate, CondPageBreak
...
class EmployeeReportPDF:
    ...
    def _create_styles(self):
        ...
        self.colors = {
            ...
            'very_light': colors.HexColor('#EEF7E5'),    # Very light green for subtle backgrounds
    ...
    def _create_page_templates(self):
        ...
        def header(canvas, doc):
            ...
            if self.logo_path and os.path.exists(self.logo_path):
                canvas.drawImage(self.logo_path, 1*cm, A4[1]-2*cm, width=2*cm, height=1*cm, preserveAspectRatio=True)
        ...
        def footer(canvas, doc):
            ...
            # Company copyright
            canvas.drawRightString(A4[0]-1.25*cm, 1*cm, f"© {datetime.now().year} {self.company_name}")
            canvas.restoreState()
    ...
    def add_metric(self, label, value, change):
        ...
        color_val = self.colors['positive'] if float(change.replace('%', '').replace('+', '')) >= 0 else self.colors['negative']
        color = 'green' if float(change.replace('%', '').replace('+', '')) >= 0 else 'red'
        change_sign = '+' if not change.startswith('+') and not change.startswith('-') and float(change.replace('%', '')) > 0 else ''
    ...
    def add_trend_chart(self, values, labels):
        ...
        # Enhanced bar chart
        r_primary, g_primary, b_primary = self.colors['primary'].red, self.colors['primary'].green, self.colors['primary'].blue
        r_dark, g_dark, b_dark = self.colors['dark'].red, self.colors['dark'].green, self.colors['dark'].blue
        
        bars = ax.bar(labels, values, color=(r_primary, g_primary, b_primary), 
                     width=0.6, edgecolor=(r_dark, g_dark, b_dark), linewidth=1)
        
        # Add trend line
        r_accent, g_accent, b_accent = self.colors['accent'].red, self.colors['accent'].green, self.colors['accent'].blue
        ax.plot(labels, values, color=(r_accent, g_accent, b_accent), 
        ...
        self.elements.append(Paragraph("<i>Figure: Employee satisfaction trend over time</i>", 
    ...
    def add_employee_table(self, data):
        ...
        self.elements.append(Paragraph("Employee Attention Report", self.styles["SectionHeader"]))
        self.elements.append(Spacer(1, 10))
        ...
        for i in range(1, len(table_data), 2):
            table_style.append(('BACKGROUND', (0, i), (-1, i), self.colors['very_light']))
            
        # Highlight negative impacts
        for i in range(1, len(table_data)):
            impact = table_data[i][2]
            if isinstance(impact, str) and ('-' in impact or 'Urgent' in impact):
                table_style.append(('TEXTCOLOR', (2, i), (2, i), self.colors['negative']))
    ...
    def add_issues_summary(self, issues, issue_counts=None):
        ...
        self.elements.append(Paragraph("Major Employee Concerns", self.styles["SectionHeader"]))
        self.elements.append(Spacer(1, 10))
        ...
        if issue_counts is None:
            ...
            issue_counts = [len(i.split()) % 10 + 5 for i in issues]  # Just a formula to get varied numbers
        ...
        bars = ax.barh(range(len(issues_short)), issue_counts, 
        ...
        ax.set_title('Major Employee Concerns by Frequency/Severity', fontsize=12, fontweight='bold', pad=15)
        ...
        self.elements.append(Paragraph("<i>Figure: Frequency/severity of major employee concerns</i>", 
                                      self.styles["CustomBodyText"]))
        self.elements.append(Spacer(1, 15))
    ...
    def add_section_divider(self):
        ...
        self.elements.append(Spacer(1, 10))
        self.elements.append(CondPageBreak(inch * 2))  # Break page if less than 2 inches left
        self.elements.append(Spacer(1, 10))
    
    def add_satisfaction_radar(self, categories, data, title="Employee Satisfaction Dimensions"):
        ...
        r, g, b = self.colors['primary'].red, self.colors['primary'].green, self.colors['primary'].blue
        ...
        self.elements.append(Paragraph("<i>Figure: Multi-dimensional employee satisfaction analysis</i>", 
                                      self.styles["CustomBodyText"]))
        self.elements.append(Spacer(1, 15))
...
def make_report(title, subtitle, metrics, issues, high_concern_employees , vibeMeterData):
    # Create a report with optional company logo
    report = EmployeeReportPDF("employee_dashboard.pdf", company_name="VibeMeter", logo_path=None)

    # Add document sections
    report.add_title(title=title)
    report.add_subtitle(subtitle=subtitle)
    metric_title = metrics['metric_title'] if 'metric_title' in metrics else "Employee Satisfaction Metrics"
